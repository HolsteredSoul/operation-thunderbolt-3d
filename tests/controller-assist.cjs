async(page)=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    g.input.source='pad';g.input.heading=0;g.input.manualAim=false;g.input.move={x:0,y:0};g.input.suspended=false;a.render();
  ...
  if(!Object.entries(result).every(([key,value])=>key==='assistedDegrees'?Math.abs(value-1.5)<.01:value))throw Error(JSON.stringify(result));
  return result;
}
