...
import {weatherGround,makeWeatherTexture} from './atmosphere.js';
const weatherTexture=makeWeatherTexture(),weatherMats=new Map(),whiteTint=new THREE.Color(1,1,1);
...
export function buildVillage(G,scene){
  ...
  const weather=weatherGround(G);scene.add(weather.mesh);
  const batch=new Batcher(scene);batch.add(root);batch.finish();return()=>{batch.clear();scene.remove(weather.mesh);weather.dispose();tex.dispose();sign.geometry.dispose();sign.material.dispose();};
}
