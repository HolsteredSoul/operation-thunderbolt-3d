...
## Mobile controls and fullscreen — 6 September 2026

`tests/mobile-controls.mjs` passed touch target retention, cover/range/death exclusion, alignment before firing, held-fire requirements, suspended input, and mouse handoff. The full `npm test` simulation suite passed all 11 groups across 20 seeds. Existing controller-assist, accessibility, and gameplay-refinements Node checks passed, including 20-seed/four-position emergency ammo checks. The 15 desktop browser checks passed after the pointer-event and canvas sizing changes; the controller-assist browser checks passed too.

`tests/mobile-browser.cjs` passed 22 checks in a fresh Chromium mobile context using CDP multi-touch events. It covered one-pad movement, simultaneous movement and firing, ammunition conservation without a target, reload, cancellation, portrait pause, deliberate landscape resume, control hit areas at 667×375, actual Chrome fullscreen entry/exit from a tap, the unsupported-fullscreen help dialog, relative manifest configuration, and zero page errors. Screenshots at 844×390, 667×375, and 390×844 were visually reviewed. The firing fixture freezes spawns, protects the player, and gives one stationary target high HP; it verifies input, not human difficulty or mobile frame rate.

The integration also avoids WebGLRenderer inline sizing expanding the mobile layout viewport after rotation, and ignores touch-generated mouse compatibility input. Pointer cancellation/lost capture, pause, resize, and focus loss clear held touches. Mobile render quality starts Low; this is not a measured phone FPS claim.

Full-screen implementation follows [MDN's user-activation requirement](https://developer.mozilla.org/en-US/docs/Web/API/Element/requestFullscreen). iOS uses the manifest and Apple home-screen metadata, with in-game instructions following [Apple's home-screen web app guide](https://support.apple.com/en-ie/guide/iphone/iphea86e5236/ios). Physical iOS home-screen launch, Android installation, safe-area behavior on notched devices, and subjective thumb feel remain unverified on hardware. Home-screen launch requires a network connection; no offline cache was added.

### iOS launch clarification

The portrait screen now exposes setup help and orientation-lock guidance. The setup dialog has numbered Safari steps, a Share icon, and fixed Close/Play actions; its Play action starts or resumes the game directly in landscape without requiring installation. Mobile menus use a fixed PLAY button. `tests/ios-launch.cjs` passed 15 launch checks with iPhone Safari identity in Chromium, at 390×664 and 667×251 (short landscape including room for browser bars). Portrait help, scrolling, direct start, pause/resume, rotation guidance, and visible action bounds passed; screenshots were reviewed. This is still browser emulation, not verification on physical Safari. Entry CSS, app and fullscreen module URLs are revisioned to refresh the changed flow for existing players.
